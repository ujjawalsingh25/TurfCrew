...
app.post('/accept', async (req, res) => {
  ...
  try { 
    const game = await Game.findById(gameId);               // Find the game
    if (!game) {
      return res.status(404).json({message: 'Game not found'});
    }
    game.players.push(userId);

    // game.requests.splice(requestIndex, 1);         // Remove the user from the requests array
...
app.post('/book', async (req, res) => {
  ...
  try {
    ...
    console.error(error);
    res.status(500).json({message: 'Server booking error'});
  }
});
...
app.post('/toggle-match-full', async (req, res) => {
  try {
    ...
    const game = await Game.findById(gameId);         // Find the game by its ID
    if (!game) {
      return res.status(404).json({ message: 'Game not found' });
    }
